...
class PlatformDetector:
    ...
    PLATFORMS = {
        ...
        'herokuapp.com': {'name': 'Heroku', 'type': 'paas', 'id': 'heroku', 'cli_tool': 'heroku'},
        'render.com': {'name': 'Render', 'type': 'paas', 'id': 'render', 'cli_tool': 'render-cli'},
        'onrender.com': {'name': 'Render', 'type': 'paas', 'id': 'render', 'cli_tool': 'render-cli'},
        'netlify.com': {'name': 'Netlify', 'type': 'paas', 'id': 'netlify', 'cli_tool': 'netlify'},
        'netlify.app': {'name': 'Netlify', 'type': 'paas', 'id': 'netlify', 'cli_tool': 'netlify'},
        'vercel.com': {'name': 'Vercel', 'type': 'paas', 'id': 'vercel', 'cli_tool': 'vercel'},
        'vercel.app': {'name': 'Vercel', 'type': 'paas', 'id': 'vercel', 'cli_tool': 'vercel'},
    ...
    def detect(url: str) -> dict:
        ...
        # SSH URL detection (git@host:user/repo.git)
        if url.startswith('git@') or url.startswith('ssh://'):
            return {'name': 'SSH Git', 'type': 'ssh_hosting', 'id': 'ssh_git', 'cli_tool': 'git'}

        # Fallback: HTTP header heuristics
        try:
            ...
            if 'vercel' in server or 'vercel' in x_powered:
                return {'name': 'Vercel', 'type': 'paas', 'id': 'vercel', 'cli_tool': 'vercel'}
            if 'heroku' in server:
                return {'name': 'Heroku', 'type': 'paas', 'id': 'heroku', 'cli_tool': 'heroku'}
            if 'cloudflare' in server:
                # Cloudflare is CDN, not hosting — fall through to unknown
                pass

            # Check HTML meta for framework hints
            soup = BeautifulSoup(response.text, 'html.parser')
            generator = soup.find('meta', attrs={'name': 'generator'})
            if generator:
                gen_content = generator.get('content', '').lower()
                if 'next.js' in gen_content or 'vercel' in gen_content:
                    return {'name': 'Vercel', 'type': 'paas', 'id': 'vercel', 'cli_tool': 'vercel'}
